...
- [ ] 2.1 `users` 모델: id, email(UNIQUE), password_hash, team_id(FK→teams NULL), created_at
- [ ] 2.2 `teams` 모델: id, name(1-30자), invite_code(UNIQUE), owner_id(FK→users), created_at
...
- [ ] 2.6 Alembic 최초 마이그레이션 생성 + `upgrade head` 검증

## 3. 인증 API (Auth 4개)

- [ ] 3.1 JWT 유틸리티: 발급(`create_access_token`), 검증(`get_current_user` 의존성), 24h 만료
...
- [ ] 4.3 `POST /teams/join`: 초대코드 형식·존재 검증, users.team_id 업데이트, 팀 미리보기 반환
- [ ] 4.4 `GET /teams/{id}`: 팀 정보 조회
- [ ] 4.5 `GET /teams/{id}/members`: 멤버 목록 (role: owner/member 구분)
- [ ] 4.6 `DELETE /teams/{id}/leave`: users.team_id = NULL 업데이트
...
- [ ] 10.1 `vercel.json`: API routes (`/api/*` → FastAPI), 정적 파일 설정
- [ ] 10.2 Vercel 프로젝트 생성 + `DATABASE_URL` 환경변수 등록 (Neon)
- [ ] 10.3 Build Command에 `alembic upgrade head` 포함
